import { Button, Grid, ListItem, ListItemButton, ListItemText, TextField } from "@mui/material"
...
export const ClassroomView = () => {
    ...
    useEffect(() => {
        const fetchClassroomData = async () => {
            try {
                const classroomData = await classroomService.getById(jwt, classroomObject.id)
    ...
    const handleGenerate = async () => {
        try {
            const updatedClassroom = await classroomService.generateClassCode(jwt, classroomObject)
            ...
            <Grid container spacing={2}>
                <Grid item xs={4}>
                    ...
                    {assignHomework ? (
                        <>
                            ...
                            >
                                {({ index, style }) => <Row index={index} style={style} quizList={quizList} handleClick={handleClick} selectedIndex={selectedIndex} />}
                            </FixedSizeList>
                ...
                <Grid item xs={4}>
                    ...
                    {classroom.students && classroom.students.map((student) =>
                        <Student key={student.id} student={student} classroom={classroom} setClassroom={setClassroom} handleClick={handleClick} />
                    )}
                </Grid>
            </Grid>
        </>

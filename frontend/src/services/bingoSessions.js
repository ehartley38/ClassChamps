...
const updateQuestions = async (jwt, sessionId, questionsArray, mistakeMade) => {
    const response = await axios.post(`${baseUrl}/updateIsCorrect/${sessionId}`, { questionsArray, mistakeMade }, {
        headers: {
            authorization: 'bearer ' + jwt.token
        }
    })
